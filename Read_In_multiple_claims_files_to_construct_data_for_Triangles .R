...
#     =================================================================================================    
#     Script Name:  Read_In_multiple_claims_files_to_construct_data_for_Triangles.R
#     =================================================================================================    
#
...
# 
#     Description:  This script was used to read in multiple historical claims files to be wrangled and 
#                   merged with notification only claims from BREEZE. This script attempts to correct 
#                   historical issues that we associated with system conversions and faulty queries that 
#                   were subsequently created to run these processes. It also is able to properly use 
#                   the same logic to split "Lost Time" and "Med Only" claims (over time different 
#                   criteria was used to split claims causing inconsistencies in data).
# 
# 
...
#     =================================================================================================    
#                                   Munging Data For Tringles 
#     =================================================================================================   
 
#   Create a function to render files into an appropriate format to be used in the loop for construction of triangles data.         
...
#   **OLD**Segragate claims based on criteria  
...
#   Identify notification only claims that are not in Ocean data (df). anti_join() is great if you need to 
#   join on multiple columns. Otherwise, subseting below should suffice.
    noti_claims_not_in_Ocean <-  anti_join( x = noti_claims, y = df, by = "CLAIM_NUMBER") 

#   Another way to do it by subsetting.  
    #noti_claims_not_in_Ocean <- noti_claims[!noti_claims$CLAIM_NUMBER %in% df$CLAIM_NUMBER, ]
...
#   This will group by and aggregate financial information by claim type.    
...
#   This will take the above financial data and combine it with claim counts.   
...
#   Loop through the file.list and read them in one by one - and bind them into dfs object.
    for (i in 1:52) {
        gc()
        print(paste0("Please Wait...Now Importing Workbook -> '", 
                     file.list[i],"' and combining it with notification only claims valued as of: ", 
                     dates[i])
              )
        df <- create_df(file.list.index = i, dates.index = i)
        
#       Some years have it as char and some as num       
        df$ACCIDENTYEAR <- as.numeric(df$ACCIDENTYEAR) 
...
#   This filters for (Q4) year-end results only. This is needed if you want to construct annual triangles. 
